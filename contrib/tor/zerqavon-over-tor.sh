...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/zerqavond"
  then
    zerqavond="$dir/zerqavond"
    break
  fi
done
if test -z "$zerqavond"
then
  echo "zerqavond not found"
  exit 1
fi
echo "Found: $zerqavond"

TORDIR="$DIR/zerqavon-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting zerqavond"
  tail -n 400 "$HOME/.bitmonero/bitmonero.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Zerqavon\ \'" | tail -n 20
  exit 1
fi
